...
- 目标：在 Agents 面板的项目名前用单个状态位置替换原生圆点。`working` 显示黄色 `⣾ ⣽ ⣻ ⢿ ⡿ ⣟ ⣯ ⣷` 动画；`blocked` 为红色实心点、`done` 为绿色实心点、`idle` 为灰色空心圆、`unknown` 为灰色小点。保留三行布局。
...
- 状态：每秒读取一次真实状态；进入 `working` 开始旋转，离开则替换为静态标记。读取或更新失败时记录错误并退出，残余图标由 TTL 清除。
...
预期显示 `PASS`，覆盖工作帧推进、静态状态替换、单位置互斥、API 失败后不再发送帧、配置边界，以及管道响应分片和异常断开。
