...
- 目标：在 Agents 面板的项目名前用单个状态位置替换原生圆点。`working` 显示黄色 `⣾ ⣽ ⣻ ⢿ ⡿ ⣟ ⣯ ⣷` 动画；`blocked` 为闪动的 `⚠︎`（U+26A0 U+FE0E）、`done` 为缓慢闪动的绿色 `✅`（U+2705）、`idle` 为灰色空心圆、`unknown` 为灰色小点。保留三行布局。
...
`✅` 使用绿色 emoji 完成图标，实际字形由终端字体渲染；`⚠︎` 用 U+FE0E 请求文本字形，由 `spin_blocked` 的红色 `fg` 染色。熄灭时分别使用两个、一个盲文空白字符（U+2800），保留各自占位。
