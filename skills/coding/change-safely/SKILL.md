---
name: change-safely
description: 修改已有代码的安全边界。改功能、修样式、调参数、重构、升级依赖等任何触碰现有代码的任务使用；先说影响面，不顺手重构，交付时只对值得完全理解的部分给位置和理由、对以后会用到的部分给契约，每个验证通过的步骤一个 commit，附用户可跑的验收命令。
license: MIT
metadata:
  category: coding
  version: "0.1"
...
- NEVER 顺手重构、顺手改格式、顺手升级依赖；发现问题单独列出，另起任务。
